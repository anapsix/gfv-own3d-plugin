...
my $video_re = qr!http[-:/a-z0-9%_.?=&]+@{[EXTENSIONS]}
                  ...
                  (?:\?[-:/a-z0-9%_.?=&]+)?!xi;
my $OWN3D = 'www\.own3d\.tv/video/';
my $OWN3D_CDN = 'vodcdn.ec.own3d.tv';
...
sub find_video {
  ...
  $actual_url =~ s/$OWN3D\d+/$OWN3D_CDN/g;
  $actual_url = $actual_url . $OWN3D_AUTH;
...
sub find_file_param {
  ...
  for my $file($param =~ /(?:video|movie|file|path)_?(?:href|src|url)?['"]?\s*[=:,]\s*['"]?([^&'" ]+)/gi,
    ...
    if($actual_url) {
      # Commented out to autodetect of filename for own3D.tv
      #my $possible_filename = $filename || (split /\//, $actual_url)[-1];
      my $possible_filename =~ m/'own3D_tv'/ || (split /\//, $actual_url)[-1];
